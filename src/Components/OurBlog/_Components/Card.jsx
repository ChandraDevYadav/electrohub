...
const Card = ({ image, date, comments, title, buttonLabel }) => {
    return (
        <div className="max-w-sm rounded-lg overflow-hidden shadow-lg m-0 md:m-4 bg-[#2a2929] relative px-5 py-5">
            <div className="relative group">
                <img src={image} alt={title} className="w-full object-cover rounded-lg" />

                {/* Left Overlay */}
                <div className="absolute inset-y-0 left-0 w-[49%] bg-red-600 opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-50">
                    
                </div>

                {/* Right Overlay */}
                <div className="absolute inset-y-0 right-0 w-1/2 bg-red-600 opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-50"></div>
            ...
            <div className=" mt-5">
                <div className="flex justify-between items-start">
                    <p className="text-white text-lg mb-2 flex justify-center font-medium items-center gap-1">
                        <BiCalendar className='text-red-600' /> {date}
                    </p>
                    <p className="text-white text-lg mb-4 flex justify-center font-medium items-center gap-1">
